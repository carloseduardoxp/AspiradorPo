...
public abstract class Busca {
	...
	protected abstract Acao[] getSequenciaAcoes();
	...
	public Estado executaBuscaCega(Estado estado) {
		...
		if (!estadosVisitados.contains(estado)) {
			for (Acao acao : getSequenciaAcoes()) {
				adicionaEstadoAVisitar(getEstado(acao, estado));
			}
			estadosVisitados.add(estado);
			observavel.notifica(estadosVisitados.size() + " estados visitados");
		}
	...
	private Estado getEstado(Acao acao, Estado estado) {
		...
		if (estado.getPosicaoX() + deslocamentoX < 0 || estado.getPosicaoX() + deslocamentoX > Agente.posX - 1) {
			return null;
		}
		if (estado.getPosicaoY() + deslocamentoY < 0 || estado.getPosicaoY() + deslocamentoY > Agente.posY - 1) {
			return null;
		}
		StatusAmbiente[][] status = estado.getStatusAmbienteClone();
		if (acao == Acao.ASPIRAR) {
			status[estado.getPosicaoX() + deslocamentoX][estado.getPosicaoY() + deslocamentoY] = StatusAmbiente.LIMPO;
		}
		return new Estado(estado, estado.getPosicaoX() + deslocamentoX, estado.getPosicaoY() + deslocamentoY, acao,
				status);
	}

	public Set<Estado> getEstadosVisitados() {
